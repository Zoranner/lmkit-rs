...
其他厂商在工厂中未实现重排序时返回 `Unsupported`（`capability` 为 `rerank`）；若选了阿里云或智谱但未启用对应厂商 feature，则返回 `ProviderDisabled`。
...
工厂阶段：`Ollama` / `Zhipu` 返回 `Unsupported`（`capability` 为 `image`）；选了 `OpenAI` 或 `Aliyun` 但未启用对应厂商 feature 时返回 `ProviderDisabled`。
